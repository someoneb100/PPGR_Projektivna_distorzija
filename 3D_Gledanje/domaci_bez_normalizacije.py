...
def provera_validnosti_originala_i_slika(originali, slike):
...
def CameraDLP(originali, slike):
    provera_validnosti_originala_i_slika(originali, slike)
    kor = korespodencije(originali, slike)
    _, _, vt = np.linalg.svd(kor, full_matrices=True)
    v = vt[-1]
